...
import { ArrowLeft, Code2, Languages, LoaderCircle, Share2, ShieldAlert, Timer, Trophy, Zap } from "lucide-react";
...
type Board = "snippet" | "timed" | "language";
...
export default function Leaderboard() {
  ...
  const [languageMode, setLanguageMode] = useState<"snippet" | "timed">("snippet");
  ...
  useEffect(() => {
    ...
    void listLeaderboard({
      ...
      mode: board === "snippet" ? "snippet" : board === "timed" ? "timed" : languageMode,
      snippetLength: board === "snippet" || (board === "language" && languageMode === "snippet") ? snippetLength : undefined,
      durationSeconds: board === "timed" || (board === "language" && languageMode === "timed") ? duration : undefined,
    }).then(async (nextRuns) => {
  ...
  }, [board, language, languageMode, snippetLength, duration]);

  const formatLabel = languageMode === "snippet" ? `${snippetLength} snippet` : `${duration}s timed`;
  const boardTitle = board === "snippet" ? `${snippetLength.charAt(0).toUpperCase()}${snippetLength.slice(1)} snippet ranking` : board === "timed" ? `${duration}s timed ranking` : `${language} · ${formatLabel} ranking`;
      ...
      <div className="relative mx-auto max-w-5xl px-4 py-8 sm:px-6 sm:py-14">
        ...
        <section className="mb-6 flex flex-wrap items-center gap-3 border-b pb-5">
            <div className="mr-auto">
              ...
              <p className="text-xs text-muted-foreground">Scores only compete with the same challenge format.</p>
            </div>
            {board === "snippet" ? (
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <div className="flex gap-1 rounded-lg border bg-card/70 p-1">
                  {(["snippet", "timed"] as const).map((item) => <button key={item} type="button" onClick={() => setLanguageMode(item)} className={`rounded-md px-3 py-2 text-xs font-medium capitalize transition-colors ${languageMode === item ? "bg-foreground text-background" : "text-muted-foreground hover:bg-muted"}`}>{item}</button>)}
                </div>
                <div className="flex gap-1 rounded-lg border bg-card/70 p-1">
                  {languageMode === "snippet"
                    ? (["short", "medium", "long"] as SnippetLength[]).map((item) => <button key={item} type="button" onClick={() => setSnippetLength(item)} className={`rounded-md px-2.5 py-2 text-xs font-medium capitalize transition-colors ${snippetLength === item ? "bg-foreground text-background" : "text-muted-foreground hover:bg-muted"}`}>{item}</button>)
                ...
                return (
                  <article key={run.$id} className={`relative overflow-hidden rounded-2xl border bg-card/85 px-2 pb-4 pt-5 text-center backdrop-blur-sm transition-transform hover:-translate-y-1 sm:px-4 ${first ? "min-h-64 border-foreground/35 shadow-[0_20px_70px_hsl(0_0%_100%/0.08)]" : "min-h-52"}`}>
                    ...
                    <Link to={`/profile/${run.userId}`} className="block">
                      ...
                      <div className="mb-2 flex justify-center"><RankMark rank={actualRank} /></div>
                      ...
                      <div className="mt-3 text-[10px] text-muted-foreground">{run.language} · {run.snippetLength ?? (run.durationSeconds ? `${run.durationSeconds}s` : run.mode)}</div>
                    </Link>
